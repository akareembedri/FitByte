...
//  TWBR = 12;  // 400 kbit/sec I2C speed
  ...
  Wire1.begin(I2C_MASTER, 0x00, I2C_PINS_37_38, I2C_PULLUP_EXT, I2C_RATE_1500);//,I2C_OP_MODE_DMA);
...
//  Serial.println("loop");
//  
  //if (readByte(MPU9250_ADDRESS, INT_STATUS) >= 0x10) {  // On interrupt, check if FIFO data ready interrupt
    // Serial.print("int ");
    //Serial.println(readByte(MPU9250_ADDRESS, INT_STATUS));
   //readFIFOcount(fifoCount);
    //Serial.print(micros());
   // Serial.print(" ");
  //  Serial.println(fifoCount[0]);
    readFIFOAll(fifoData);
    for (int i=0; i<_aSize;i++)
    {
    //  Serial.print(micros());
  // Serial.print(" ");
    Serial.println(fifoData[i]);
      delay(0.25);
...
//
//   // Wire1.sendRequest(MPU9250_ADDRESS,1, I2C_STOP);  // NON-blocking read (request 256 bytes)
//    buff[cn] = accelCount[2];
//
//    Serial.println(cn++);
//    Serial.print("buff : ");
//    Serial.println(buff[cn]);
    //delay(100);
//      }
//       Wire1.finish();
//       Serial.println("wire finished");
//  for (int e;e<60;e++){
//    Serial.println(buff[e]);}  
...
void readFIFOcount(int16_t * destination)
{
  ...
  readBytes(MPU9250_ADDRESS, FIFO_COUNTH, 2, &rawData[0]);  // Read the six raw data registers into data array
...
int16_t fifoCount[1];  // Stores the 16-bit signed accelerometer sensor output
  ...
  readFIFOcount(fifoCount);
   ...
   for (size_t i=0; i < fifoCount[0]/_fifoFrameSize; i++)
   {
    ...
    readBytes(MPU9250_ADDRESS, FIFO_R_W, _fifoFrameSize, &_buffer[0]);  // Read the six raw data registers into data array
     ...
     //   _axFifo[i] = (float)_axcounts;//(((float)(tX[0]*_axcounts + tX[1]*_aycounts + tX[2]*_azcounts) * _accelScale)-_axb)*_axs;
 ...
 // readBytes(MPU9250_ADDRESS, FIFO_R_W, 500, &rawData[0]);  // Read the six raw data registers into data array
  //destination[0] = ((int16_t)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
...
//  {
//    //int d=(int)(i/2);
//      destination[i] =rawData[i] ;  // Turn the MSB and LSB into a signed 16-bit value
//
//    }
//  for (int i=0;i<500;i+=2)
//  {
//    int d=(int)(i/2);
//      destination[d] = ((int16_t)rawData[i] << 8) | rawData[i+1] ;  // Turn the MSB and LSB into a signed 16-bit value
//
//    }
}
...
{  
 ...
 // get stable time source
  ...
  delay(200); 
  //----------------fifo------------------------------
  writeByte(MPU9250_ADDRESS, USER_CTRL, 0x40|0x20);
  writeByte(MPU9250_ADDRESS, FIFO_EN, 0x08);      // enable FIFO Accele
...
// Set gyroscope full scale range
 ...
 // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
  ...
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2,0x08);// 0x08); // Write new ACCEL_CONFIG2 register value torun at 4KHz
 ...
 // but all these rates are further reduced by a factor of 5 to 200 Hz because of the SMPLRT_DIV setting
  ...
  // can join the I2C bus and all can be controlled by the Arduino as master
   ...
   writeByte(MPU9250_ADDRESS, INT_ENABLE, 0x10);// Enable data ready (bit 0) interrupt
   writeByte(MPU9250_ADDRESS, PWR_MGMT_2, 0b00000111); //disable magnitomiter 
   delay(100);
}
